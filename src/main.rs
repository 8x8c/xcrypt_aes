...
use aes_gcm::{
    ...
    Key,          // 256-bit key for AES-256-GCM
    Nonce         // GCM supports 96-bit (12-byte) nonce
};
...
/// Magic header to identify AES-256-GCM encrypted files (8 bytes).
/// Adjust if you want a different marker or want to preserve old magic.
const MAGIC: &[u8; 8] = b"AES256GC";
...
fn main() -> std::io::Result<()> {
    ...
    if !is_encrypted {
        ...
        // Encrypt in-place
        // GCM appends a 16-byte tag at the end of the buffer
        ...
        buffer.drain(0..MAGIC.len()+16+12);

        // Decrypt in-place
        if let Err(_) = cipher.decrypt_in_place(nonce, b"", &mut buffer) {
            key_bytes.zeroize();
            password_bytes.zeroize();
            buffer.zeroize();
